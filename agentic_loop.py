...
async def universal_agent_loop(
    ...
    *,
    ...
    """Run extraction -> validation loop and adapt instructions after failures."""
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        for attempt in range(1, max_attempts + 1):
            ...
            print(f'Current extraction payload: {json.dumps(extraction_payload, indent=2)}')
            print(f"Attempt {attempt}: Sending extraction request with instructions:\n{current_instructions}\n")
            ...
            print(f"Attempt {attempt} failed validation: {last_feedback}")

            # Feed validation feedback back into extraction instructions.
            current_instructions = (
                f"{base_instructions}\n\n"
                "--- ATTENTION: PREVIOUS ATTEMPT FAILED ---\n"
                "Your last extraction was rejected for the following reasons:\n"
                f"{last_feedback}\n"
                "Please consider the failure of the reasons above and adjust your extraction accordingly. "
                "You can adjust the description of the fields, but you cannot change the field names or types."
            )
